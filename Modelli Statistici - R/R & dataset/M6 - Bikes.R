...
### Vediamo cosa succede se usassimo priors informative
...
fit_glm1_inf = jags(
			...
			inits 							= inits,
			parameters.to.save 	= SavePar,
			model.file 					= textConnection(mod1_string_inf), # il model.file dovrebbe essere un file,
			 																									# "textConnection" crea il file e lo carica
			n.chains 						= nchain,
			n.iter 							= 10000,
			n.burnin 						= 5000,
...
#### stime
#### #### #### #### #### #### ####
beta_post_chain1_inf = fit_glm1_inf$BUGSoutput$sims.array[-c(1:100),1,1:p]
beta_post_chain2_inf = fit_glm1_inf$BUGSoutput$sims.array[-c(1:100),2,1:p]
colnames(beta_post_chain1_inf) = colnames(beta_post_chain2_inf) = colnames(X)

# confrontiamo le catene per vedere se il modello è a convergenza
par(mfrow=c(3,2))
for(i in 1:p)
{
	plot(beta_post_chain1_inf[,i], type="l", main= colnames(beta_post_chain1_inf)[i])
	lines(beta_post_chain2_inf[,i],col=2)
	# if(p%%6)
	# {
	# 	plot.new()
	# }
...
# stimiamo le medie e gli intervalli di credibilità
beta_mean_inf = round(colMeans(beta_post_chain1_inf),5)
beta_CI1_inf  = round(apply(beta_post_chain1_inf,2,quantile,prob=0.025),5)
beta_CI2_inf  = round(apply(beta_post_chain1_inf,2,quantile,prob=0.975),5)

Beta_Res_inf = cbind(beta_mean_inf,beta_CI1_inf,beta_CI2_inf)
Beta_Res_inf

cbind(Beta_Res_inf,Beta_Res)
# abbiamo lo stesso problema interpretativo dovuto alla collinearità
...
set.seed(123)
inits =  list(
  list("b_app" = runif(0,-1,1), "z" = rep(1,p), "prob"=rep(0.5,3)),
  list("b_app" = runif(0,-1,1), "z" = rep(1,p), "prob"=rep(0.5,3))
)
...
z_post = fit_glm2$BUGSoutput$sims.array[,2,c("z[1]","z[8]","z[10]","z[11]","z[12]")]
colnames(z_post) = c("Season","Temperature","Rain","Snow","Time")
colMeans(z_post)
...
# Il valore di low temp non è significativo nel modelo generale, non lo è
# se condiziono solo quando z non lo sceglie come trasformazione, ma lo è quando
# z della temparatura sceglie il low (ma questa stima si basa con solo poche iterazioni, quindi poco affidabile)
# i.e.
sum(z_post[,"Temperature"]==0)
### ### ### ### ### ### ### ### ###
### facciamo la stessa analisi ma con una prior su più informativa e vicino a 0 per le probabilità
mod4_string <- 'model {
  for(i in 1:n) {
    Y[i]  ~  dpois(lambda[i])
		lambda[i] = exp(mu[i])
	...
	prob[2] ~ dbeta(0.0001,1) ## preferisco low-temp
	prob[3] ~ dbeta(0.0001,1) ## preferisco time^0.5
